    ...
    for (selector of sortedSelectors) {
        ...
        for (selectedEl of selector.selectedEls) {
            ...
            var firstSubSelector = selector.item.data.children.first(),
                className = firstSubSelector.name;

            if(firstSubSelector.type === 'ClassSelector') {
                selectedEl.class.remove(className);
                if (className.indexOf("themed-") >= 0) {

                    selectedEl.class.add("themed");
                    selectedEl.class.add(className.substring("themed-".length))
                }
            }
